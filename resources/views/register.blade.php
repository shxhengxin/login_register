...
@section('content')
    ...
    <div class="weui_cells weui_cells_form" style="display: none;">
        <div class="weui_cell">
            ...
            <div class="weui_cell_bd weui_cell_primary">
                <input class="weui_input" type="text" placeholder="" name='email' id="email"/>
            </div>
        </div>
        <div class="weui_cell">
            <div class="weui_cell_hd"><label class="weui_label">密码</label></div>
...
<script type="text/javascript">
    ...
    function verifyPhone(phone, password, confirm, phone_code) {
        ...
        if(confirm.length < 6) {
            $('.bk_toptips').show();
            $('.bk_toptips span').html('确认密码不能少于6位');
            setTimeout(function() {$('.bk_toptips').hide();}, 2000);
            return false;
        }
        if(password != confirm) {
...
<script>
    $("#phone").blur(function () {
        ...
        if($(this).val().length != 11  || $(this).val()[0] != '1'){
            $('.bk_toptips').show();
            $('.bk_toptips span').html('输入的手机号格式错误');
            setTimeout(function() {$('.bk_toptips').hide();},5000);
            return;
        }
        var phone = $("#phone").val();
        $.ajax({
            url: '/service/check/phone',
            dataType: 'json',
            cache: false,
            type: 'POST',
            data: {phone: phone,_token:'{{ csrf_token() }}'},
...
<script>
    $("#email").blur(function(){
        //验证邮箱
            ...
            if(!email.match(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/)){
                $('.bk_toptips').show();
                $('.bk_toptips span').html("请输入正确邮箱格式");
                setTimeout(function () {$('.bk_toptips').hide();},5000);
                return;
            }
        var email = $("#email").val();
        $.ajax({
            url:'/service/check/email',//检查该邮箱是否已注册
            typeData:'json',
            type:'POST',
            cache: false,
            data: {phone:email ,_token:'{{ csrf_token() }}'},
            success: function(data) {
                if(data == null){
                    $(".bk_toptips").show();
